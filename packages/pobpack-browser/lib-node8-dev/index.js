...
const createAppBrowserCompiler = exports.createAppBrowserCompiler = (target, options, compilerOptions) => {
  ...
  return _flowRuntime2.default.param('target', _targetType).assert(target), _flowRuntime2.default.param('options', _optionsType).assert(options), _returnType.assert((0, _pobpackUtils.createPobpackCompiler)(target, (0, _pobpackUtils.createAppWebpackConfig)((0, _createBrowserWebpackConfig2.default)(target))(Object.assign({}, options, {
...
const watch = exports.watch = (options, callback) => {
  ...
  _flowRuntime2.default.param('callback', _callbackType).assert(callback), typeof options === 'function' && (callback = _callbackType.assert(options), options = undefined);
...
const runDevServer = (compiler, options) => {
  ...
  _flowRuntime2.default.param('compiler', _compilerType).assert(compiler), _flowRuntime2.default.param('options', RunOptions).assert(options);
...
const watchAndRunDevServer = exports.watchAndRunDevServer = (options, runOptions) => {
  ...
  _flowRuntime2.default.param('options', _optionsType3).assert(options), _flowRuntime2.default.param('runOptions', RunOptions).assert(runOptions);
